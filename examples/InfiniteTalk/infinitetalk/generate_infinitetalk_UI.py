...
def generate(args):
    ...
    pipename = f"infinitetalk_fifo_{rank}"
    while(not os.path.exists(pipename)):
        time.sleep(1)
    ...
    if rank == 0:
        ...
        while(not os.path.exists(pipename)):
            time.sleep(1)
        result_pipe = os.open(pipename, os.O_WRONLY)

    args.audio_save_dir = args.audio_save_dir + f'_cp{rank}'
    while(True):
        data = os.read(pipe, 16384).decode()
        ui_args = json.loads(data)
        print("rank: ", rank)
        print(ui_args)

        input_data = ui_args['input_data']
        ...
        for idx, items in enumerate(zip(*conds_list)):
            ...
            video = wan_i2v.generate_infinitetalk(
                ...
                sampling_steps=ui_args.get('sampling_steps', args.sample_steps),
                text_guide_scale=ui_args.get('text_guide_scale', args.sample_text_guide_scale),
                audio_guide_scale=ui_args.get('audio_guide_scale', args.sample_audio_guide_scale),
                seed=ui_args.get('seed', args.base_seed),
                offload_model=args.offload_model,
                max_frames_num=ui_args.get('max_frames_num', max_frames_num),
                color_correction_strength = args.color_correction_strength,
                extra_args=args,
                n_prompt=ui_args.get('n_prompt', ''),
                )
        ...
        if rank == 0:
            ...
            os.write(result_pipe, f"{args.save_file}.mp4".encode())
