...
def _parse_args():
    ...
    parser.add_argument(
        "--lora_dir", type=str, nargs="+", default=None, help="The path to the LoRA checkpoint directory."
    )
    parser.add_argument(
        "--lora_scale",
        type=float,
    ...
    parser.add_argument(
        "--audio_save_dir", type=str, default="save_audio/gradio", help="The path to save the audio embedding."
    )
        ...
        if "Local File" in mode_selector:
            ...
            if "human2_voice" not in input_data["tts_audio"].keys():
                new_human_speech1, sum_audio = process_tts_single(
                    input_data["tts_audio"]["text"], args.audio_save_dir, input_data["tts_audio"]["human1_voice"]
                )
                audio_embedding_1 = get_embedding(new_human_speech1, wav2vec_feature_extractor, audio_encoder)
                emb1_path = os.path.join(args.audio_save_dir, "1.pt")
                torch.save(audio_embedding_1, emb1_path)
    ...
    with gr.Blocks() as demo:
        ...
        with gr.Row():
            with gr.Column(scale=1):
                ...
                vid2vid_vid = gr.Video(label="Upload Input Video", visible=True)
                img2vid_image = gr.Image(
                    type="filepath", label="Upload Input Image", elem_id="image_upload", visible=False
                )
                img2vid_prompt = gr.Textbox(
                    label="Prompt",
                    placeholder="Describe the video you want to generate",
                )
                task_mode.change(fn=show_upload, inputs=task_mode, outputs=[img2vid_image, vid2vid_vid])
                ...
                with gr.Accordion("Audio Options", open=True):
                    ...
                    img2vid_audio_2 = gr.Audio(
                        label="Conditioning Audio for speaker 2", type="filepath", visible=False
                    )
                    tts_text = gr.Textbox(
                        label="Text for TTS",
                ...
                with gr.Accordion("Advanced Options", open=False):
                    with gr.Row():
                        sd_steps = gr.Slider(label="Diffusion steps", minimum=1, maximum=1000, value=8, step=1)
                        seed = gr.Slider(label="Seed", minimum=-1, maximum=2147483647, step=1, value=42)
                    with gr.Row():
                        text_guide_scale = gr.Slider(
                            label="Text Guide scale", minimum=0, maximum=20, value=1.0, step=1
                        )
                        audio_guide_scale = gr.Slider(
                            label="Audio Guide scale", minimum=0, maximum=20, value=2.0, step=1
                        )
                    with gr.Row():
                        ...
                        human2_voice = gr.Textbox(
                            label="Voice for right person", value="weights/Kokoro-82M/voices/af_heart.pt"
                        )
                    # with gr.Row():
                    n_prompt = gr.Textbox(
